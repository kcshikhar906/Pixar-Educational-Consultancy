...
const selectableCountries = [ // This list is for the Document Checklist
...
export default function SmartToolsPage() {
  ...
  // State for Document Checklist (Remains unchanged)
  ...
  const docChecklistForm = useForm<DocumentChecklistFormValues>({ // Unchanged
  ...
  async function onDocChecklistSubmit(values: DocumentChecklistFormValues) { // Unchanged
  ...
  const handleDownloadPdf = () => { // Unchanged
          ...
          <TabsContent value="sop-generator">
            <div className={cn(
              ...
              <Card className={cn(
                  ...
                  showSopResultsArea ? "md:col-span-1" : "max-w-3xl" // Wider for SOP form
              )}>
                <CardHeader>
                  ...
                  <form onSubmit={sopForm.handleSubmit(onSopSubmit)}>
                    <CardContent className="space-y-6">
                      ...
                      <FormField control={sopForm.control} name="fieldOfStudy" render={({ field }) => (
                        <FormItem><FormLabel>Desired Field of Study</FormLabel><FormControl><Input placeholder="e.g., Computer Science, Business Analytics" {...field} /></FormControl><FormMessage /></FormItem>
                      )}/>
                      <FormField control={sopForm.control} name="academicBackground" render={({ field }) => (
                        <FormItem><FormLabel>Academic Background</FormLabel><FormControl><Textarea placeholder="Detail your past education, relevant subjects, grades/GPA, key projects, research, and academic achievements..." rows={5} {...field} /></FormControl><FormMessage /></FormItem>
                      )}/>
                      <FormField control={sopForm.control} name="extracurricularsWorkExperience" render={({ field }) => (
                        <FormItem><FormLabel>Extracurriculars & Work Experience (Optional)</FormLabel><FormControl><Textarea placeholder="Describe relevant work experience, internships, volunteer work, leadership roles, or significant extracurricular activities..." rows={4} {...field} /></FormControl><FormMessage /></FormItem>
                      )}/>
                      <FormField control={sopForm.control} name="whyThisProgram" render={({ field }) => (
                        <FormItem><FormLabel>Why This Program/University?</FormLabel><FormControl><Textarea placeholder="Explain your reasons for choosing this specific program and university. Mention specific aspects like curriculum, faculty, research opportunities if known..." rows={4} {...field} /></FormControl><FormMessage /></FormItem>
                      )}/>
                      <FormField control={sopForm.control} name="whyThisCountry" render={({ field }) => (
                        <FormItem><FormLabel>Why This Country?</FormLabel><FormControl><Textarea placeholder="Explain your reasons for choosing this particular country for your studies. Consider academic environment, cultural aspects, career opportunities, etc..." rows={3} {...field} /></FormControl><FormMessage /></FormItem>
                      )}/>
